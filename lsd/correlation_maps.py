...
for hemi in ['lh', 'rh']:
    ...
    file_list = glob.glob('%s/*/resting/*_lsd_rest*_fsaverage5_%s.mgz' % (baseDir, hemi))    
    for n, restFile in enumerate(file_list):
        
        corrFile = restFile.replace('resting', 'correlation_maps').replace('rest', 'corr_').replace('fsaverage5', 'fsa5').replace('.mgz', '')
